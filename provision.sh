...
function print_usage() {
  echo "$(basename "$0") - Create, stop, and destroy nodes using vagrant
...
case "$ACTION" in
	...
	do
	    if [[ i -eq 1 ]]; then
	        NODES=$(ls -1U ./resources/.vagrant/machines/ | wc -l) vagrant ssh node${i} -c "ip addr show eth1" | grep -o -E "inet [0-9]+(\.[0-9]+){3}" | cut -d " " -f 2- >  ./nodes
	    else
	        NODES=$(ls -1U ./resources/.vagrant/machines/ | wc -l) vagrant ssh node${i} -c "ip addr show eth1" | grep -o -E "inet [0-9]+(\.[0-9]+){3}" | cut -d " " -f 2- >>  ./nodes
	    fi
	done
